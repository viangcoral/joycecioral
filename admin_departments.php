    ...
    if (isset($_POST['add_department'])) {
        ...
        if (empty($name)) {
            ...
            try {
                ...
                $messageType = 'success';
                logActivity('add_department', "Added department: $name");
        ...
        if (empty($name)) {
            ...
            try {
                ...
                $messageType = 'success';
                logActivity('edit_department', "Updated department ID: $id");
            } catch(PDOException $e) {
        ...
        try {
            ...
            if (!$deptName) {
                ...
                // Check if department is being used by programs or users
                ...
                if ($programCount > 0 || $userCount > 0) {
                    $message = 'Cannot delete department "' . htmlspecialchars($deptName) . '". It is being used by ' . $programCount . ' program(s) and ' . $userCount . ' user(s). Please remove all associated programs and users first.';
                    $messageType = 'error';
                } else {
                    ...
                    $messageType = 'success';
                    logActivity('delete_department', "Deleted department: $deptName");
                }
            }
        } catch(PDOException $e) {
...
<head>
    ...
    <title>Manage Departments - DDBQAAMS</title>
    ...
    <div class="dashboard">
        <nav class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="admin_departments.php" class="active">Department Management</a></li>
                ...
                <div class="card" style="grid-column: 1 / -1;">
                    ...
                    <div class="card-body">
                        ...
                        <div style="margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid #e6e6e6;">
                            <h4 style="margin-top: 0; color: #1e3a8a;">Add New Department</h4>
                        ...
                        <div>
                            <h4 style="margin-top: 0; color: #1e3a8a;">Existing Departments</h4>
                                    ...
                                    <tbody>
                                        <?php foreach ($departments as $dept): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-secondary" onclick="editDepartment(<?php echo $dept['id']; ?>, '<?php echo addslashes($dept['name']); ?>', '<?php echo addslashes($dept['description'] ?? ''); ?>')">Edit</button>
                                                    <button class="btn btn-sm btn-danger" onclick="deleteDepartment(<?php echo $dept['id']; ?>, '<?php echo addslashes($dept['name']); ?>')">Delete</button>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
    ...
    <div id="editModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <form method="POST" class="modal-body">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="edit_parent_id">Parent Department</label>
                        <select id="edit_parent_id" name="parent_id">
                            <option value="">Select Parent Department (Optional)</option>
                            <?php foreach ($all_departments as $pid => $pname): ?>
                                <option value="<?php echo $pid; ?>"><?php echo htmlspecialchars($pname); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="edit_head_user_id">Department Head</label>
                        <select id="edit_head_user_id" name="head_user_id">
                            <option value="">Select Department Head (Optional)</option>
                            <?php foreach ($potential_heads as $head): ?>
                                <option value="<?php echo $head['id']; ?>"><?php echo htmlspecialchars($head['name'] . ' (' . $head['role_name'] . ')'); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
    ...
    <div id="deleteModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5);">
        <div class="modal-content" style="background-color: #fefefe; margin: 15% auto; padding: 0; border: 1px solid #888; width: 80%; max-width: 500px; border-radius: 8px;">
            ...
            <div class="modal-body" style="padding: 20px;">
                <p>Are you sure you want to delete the department "<span id="delete_dept_name" style="font-weight: bold;"></span>"?</p>
            ...
            <div class="modal-footer" style="padding: 15px; background-color: #f8f9fa; border-top: 1px solid #dee2e6; border-radius: 0 0 8px 8px; text-align: right;">
                ...
                <form method="POST" style="display: inline;">
                    ...
                    <button type="submit" name="delete_department" class="btn btn-danger" style="padding: 8px 16px; background-color: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer;">Delete Department</button>
                </form>
            </div>
        </div>
    </div>
